...
- **博客**：文章列表、详情阅读、富文本展示
- **动态**：时间流式内容展示与详情查看，详情页会校验路由参数，非法地址会直接提示而不会继续请求错误接口
- **留言**：访客留言互动
...
- **系统配置**：存储方式、OSS 参数、IP 查询方案配置、首页公告
- **用户管理**：用户 CRUD、角色权限管理

## 访问日志与 IP 查询配置

- 后台系统配置页现已改为 **标签页（Tabs）**，IP 查询配置集中在独立标签中
- IP 查询支持两种二选一方案：
  - `xxapi`：读取返回 JSON 中的 `data.address`
  - `ipdatacloud`：请求时额外携带 `key`，读取 `multi_street` 第一个对象，并按“国家 + 省份 + 市 + 区 + 街道”拼接实际地址
- `ipdatacloud` 需要同时配置请求地址与接口密钥
- 启用后，后端会在写入访问日志时调用当前选中的查询方案
- 相同 IP 会优先复用 **当前查询方案** 下的缓存与历史日志结果，避免对同一访客重复请求第三方接口
- 当系统配置开启 IP 查询并保存时，后端会自动批量回填历史访问日志里缺失的 `actual_address`
- 历史回填会先按 IP 去重，再逐个 IP 查询；同一个 IP 在同一查询方案下最多请求一次，随后批量填充该 IP 的所有空地址记录
- 不同查询方案的地址结果会隔离存储，不会复用另一种方案已解析出的地址
